...
impl ObjPool {
    ...
    pub fn add_child(&mut self, parent: Obj, child: Obj) {
        self.parent[child.id] = Some(parent);

        if let Some(first_child) = self.left[parent.id] {
            // Parent has children. Find next available sibling attribute in children.
            let mut c = first_child;
            while let Some(sibling) = self.right[c.id] {
                c = sibling;
            }
            self.right[c.id] = Some(child);
        } else {
            // First child. Set parent's child attribute.
            self.left[parent.id] = Some(child);
        }
    }
    ...
    pub fn add_csg(&mut self, op: CsgOp, transform: Matrix<4>, left: Obj, right: Obj) -> Obj {
        ...
        self.parent[left.id] = Some(csg);
        self.parent[right.id] = Some(csg);
        self.left[csg.id] = Some(left);
        self.right[csg.id] = Some(right);
        csg
    }
    ...
    pub fn intersect(&self, ray: &Ray) -> Vec<Intersection> {
        ...
        fn intersect_rec(obj_pool: &ObjPool, root: Obj, ray: &Ray, xs: &mut Vec<Intersection>) {
            let ray = ray.transform(obj_pool.transform_inverse[root.id]);
            match &obj_pool.tag[root.id] {
                ObjTag::Shape(shape) => shape.intersects(ray, root, xs),
                ObjTag::Group => {
                    let mut child = obj_pool.left[root.id];
                    while let Some(c) = child {
                        intersect_rec(obj_pool, c, &ray, xs);
                        child = obj_pool.right[c.id];
                    }
                }
                ObjTag::Csg(op) => {
                    let left = obj_pool.left[root.id].unwrap();
                    let right = obj_pool.right[root.id].unwrap();
    ...
    pub fn world_to_object(&self, obj: Obj, point: Tuple) -> Tuple {
        let point = if let Some(parent) = self.parent[obj.id] {
            self.world_to_object(parent, point)
        } else {
            point
        };

        self.transform_inverse[obj.id] * point
    }

    fn normal_to_world(&self, obj: Obj, normal: Tuple) -> Tuple {
        let normal = {
            let mut n = self.transform_inverse[obj.id].transpose() * normal;
            n.set_w(0.0);
            n.normalize()
        };

        if let Some(parent) = self.parent[obj.id] {
    ...
    fn includes(&self, search_target: Obj, node: Obj) -> bool {
        ...
        match &self.tag[node.id] {
            ObjTag::Shape(_) => false,
            ObjTag::Group => {
                let mut child = self.left[node.id];
                while let Some(c) = child {
                    if self.includes(search_target, c) {
                        return true;
                    } 
                    child = self.right[c.id];
                }
                false
            }
            ObjTag::Csg(_) => {
                if let Some(left) = self.left[node.id] {
                    if self.includes(search_target, left) {
                        return true;
                    }
                }

                if let Some(right) = self.right[node.id] {
...
impl fmt::Display for ObjPool {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        fn write_rec(obj_pool: &ObjPool, f: &mut fmt::Formatter<'_>, root: Obj, depth: usize) -> fmt::Result {
            for _ in 0..depth {
                write!(f, "    ")?;
            }
            ...
            match &obj_pool.tag[root.id] {
                ...
                ObjTag::Csg(op) => {
                    writeln!(f, "CSG({:?})", op)?;
                    if let Some(left) = obj_pool.left[root.id] {
                        write_rec(obj_pool, f, left, depth + 1)?;
                    }
                    if let Some(right) = obj_pool.right[root.id] {
